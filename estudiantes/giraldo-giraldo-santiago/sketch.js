...
function setup() {
  ...
  inputTexto.parent("canvasContainer");
...
function draw() {
  ...
  intensidad = map(mouseX, 0, width, 0, 100);

  fractura = map(micLevel, 0, 0.15, 0, 100);

  fractura = constrain(fractura, 0, 100);

  // ======================================================
  // COLOR EMOCIONAL
  // ======================================================

  let textoActual = inputTexto.value().toLowerCase();
  ...
  if (glitchMode || fractura > 65) {
    ...
    image(scan1, -width * 0.1, -height * 0.1, extraW, extraH);
  ...
  let escala = map(intensidad, 0, 100, 0.8, 2);
  ...
  for (let i = 0; i < puntos.length; i++) {
    ...
    let ruido = noise(p.x * 0.01, p.y * 0.01, frameCount * 0.01);

    let movimiento = map(ruido, 0, 1, -fractura, fractura);

    let audioReact = micLevel * 1200;

    let x = p.x - width / 2 + movimiento + random(-audioReact, audioReact);

    let y = p.y - height / 2 + movimiento + random(-audioReact, audioReact);

    let respiracion = sin(frameCount * 0.05 + i) * 1.5;

    let size = map(intensidad, 0, 100, 2, 8) + respiracion;
  ...
  if (procesando) {
    ...
    text("procesando emoción...", width / 2 - 110, height / 2 + 220);
  }
...
function mostrarInterfaz() {
  ...
  text("RECONFIGURADOR EMOCIONAL", 40, 115);

  textSize(14);

  text("deja algo que no puedas controlar", 40, 150);

  text("la intensidad altera el sistema", 40, 175);

  text("tu voz fractura el sistema", 40, 200);

  text("ENTER → procesar emoción", 40, 245);

  text("G → activar glitch", 40, 270);

  text("S → guardar imagen", 40, 295);
  ...
  rect(40, height - 75, 240, 8);

  fill(red(emotionColor), green(emotionColor), blue(emotionColor));

  rect(40, height - 75, intensidad * 2.4, 8);

  fill(255);

  text("intensidad", 40, height - 85);

  // fractura

  fill(255, 40);

  rect(40, height - 40, 240, 8);

  fill(red(emotionColor), green(emotionColor), blue(emotionColor));

  rect(40, height - 40, fractura * 2.4, 8);

  fill(255);

  text("fractura", 40, height - 50);
}
...
function actualizarParticulas() {
  for (let i = particulas.length - 1; i >= 0; i--) {
    ...
    fill(red(emotionColor), green(emotionColor), blue(emotionColor), p.alpha);
...
function efectoHumo() {
  ...
  tint(red(emotionColor), green(emotionColor), blue(emotionColor), 20);

  image(smoke1, mouseX - 250, mouseY - 250, 500, 500);

  tint(red(emotionColor), green(emotionColor), blue(emotionColor), 14);

  image(smoke2, width - mouseX - 300, height - mouseY - 300, 600, 600);
...
function controlarAudio() {
  let velocidad = map(intensidad, 0, 100, 0.7, 1.4);

  ambient.rate(velocidad);

  let heartbeatRate = map(fractura, 0, 100, 0.8, 1.8);

  heartbeat.rate(heartbeatRate);

  let volumenRespiracion = map(micLevel, 0, 0.1, 0.4, 1);

  heartbeat.setVolume(volumenRespiracion);
}
...
function dibujarCursor() {
  ...
  stroke(red(emotionColor), green(emotionColor), blue(emotionColor), 150);

  strokeWeight(1);

  ellipse(mouseX, mouseY, 40 + sin(frameCount * 0.1) * 10);
